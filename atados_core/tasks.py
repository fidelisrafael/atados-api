...
def send_email_to_volunteer_after_4_weeks_of_apply(volunteer_email, project_id):
    ...
    subject, from_email, to = u'Como anda o voluntariado?', 'contato@atados.com.br', volunteer_email
...
def send_email_to_volunteer_3_days_before_pontual(volunteer_email, project_id):
    ...
    d = Context({ "project_name": project.name })
    subject, from_email, to = u'Um ato está chegando... estamos ansiosos para te ver.', 'contato@atados.com.br', volunteer_email
    text_content = plaintext.render(d)
    html_content = htmly.render(d)
    msg = EmailMultiAlternatives(subject, text_content, from_email, [to])
    msg.attach_alternative(html_content, "text/html")
